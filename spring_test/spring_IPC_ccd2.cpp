...
double nodeSegmentSignedDistance(const Vec2 &xi, const Vec2 &xj, const Vec2 &xjp1){
    ...
    if (d < 1e-12) return 0.0; // Exactly on the segment

    // Segment direction and unit normal
    Vec2 seg = { xjp1.x - xj.x, xjp1.y - xj.y };
    double seg_len = std::sqrt(seg.x * seg.x + seg.y * seg.y);
    if (seg_len < 1e-12) return d; // Degenerate segment

    Vec2 n_line = { -seg.y / seg_len, seg.x / seg_len }; // Normal of the line

    double side_sign;

    if (t <= 1e-6) {
        // Closest to xj
        Vec2 r_endpoint = {xi.x - xj.x, xi.y - xj.y};
        // Use normal of line at endpoint
        side_sign = (n_line.x * r_endpoint.x + n_line.y * r_endpoint.y) >= 0.0 ? 1.0 : -1.0;
    } else if (t >= 1.0 - 1e-6) {
        // Closest to xjp1
        Vec2 r_endpoint = {xi.x - xjp1.x, xi.y - xjp1.y};
        // Use normal of line at endpoint
        side_sign = (n_line.x * r_endpoint.x + n_line.y * r_endpoint.y) >= 0.0 ? 1.0 : -1.0;
    } else {
        // Interior
        side_sign = (n_line.x * r.x + n_line.y * r.y) >= 0.0 ? 1.0 : -1.0;
    }
...
double compute_safe_step(int who_global, const Vec2 &dx, const Vec &x_global,
    ...
    for (const auto &c : barriers_global) {
        ...
        // Zero increments for nodes NOT moving this iteration
        ...
        if (who_global == c.node) {
            ...
            dxi = full_step; // use negative for descent direction
            ...
            // barrier not affected by this DOF
            continue;
        }
...
std::vector<BarrierPair> build_barriers(int N_left, int N_right){
    std::vector<BarrierPair> barriers;

    // All nodes from the right chain vs. segments from the left
    for (int i = 0; i < N_right; ++i) {
        for (int j = 0; j < N_left - 1; ++j) {
            if ( (N_left + i == j) || (N_left + i == j+1) ) continue; // Should not happen in this setup
            barriers.push_back({N_left + i, // global index of right node
                                j,          // global index of left segment start
                                j + 1       // global index of left segment end
                               });
        }
    }

    // All nodes from the left chain vs. segments from the right
    for (int i = 0; i < N_left; ++i) {
        for (int j = 0; j < N_right - 1; ++j) {
            if ( (i == N_left + j) || (i == N_left + j + 1) ) continue; // Should not happen
            barriers.push_back({i,           // global index of left node
                                N_left + j,  // global index of right segment start
                                N_left + j + 1 // global index of right segment end
                               });
        }
    }
    return barriers;
...
int main(){
    ...
    // Create chains
    Chain left  = make_chain({-1.0, 0.0}, {0.0, -1.0}, 2, true, false, 0.05);
    Chain right = make_chain({-1.5, 0.5}, {-0.5, 0.5}, 2, true, false, 0.05);
    ...
    double prev_signed_d = std::numeric_limits<double>::quiet_NaN();
    ...
    for (int frame = 1; frame <= total_frame; ++frame) {
        ...
        for (int i = 0; i < left.N; ++i) {
            if (left.is_fixed[i]) continue;
            Vec2 xi = getXi(left.x, i);
            Vec2 vi = getXi(left.v, i);
            setXi(left.xhat, i, { xi.x + dt * vi.x, xi.y + dt * vi.y });
        }
        ...
        std::vector<BarrierPair> barriers = build_barriers(left.N, right.N);

        // Gauss–Seidel solvers
        // Left chain (global_offset = 0)
        Vec xnew_left  = left.x;
        std::pair<double, int> result_left = gauss_seidel_minimize_with_barrier_global(xnew_left, left.xhat,
        ...
        double residual_left = result_left.first;
        int sweeps_left = result_left.second;

        // Velocity update for the left chain
        for (int i = 0; i < left.N; ++i) {
            if (left.is_fixed[i]) continue;
            Vec2 xi_new = getXi(xnew_left, i);
            Vec2 xi_old = getXi(left.x, i); // Get old position
            setXi(left.v, i, { (xi_new.x - xi_old.x) / dt, (xi_new.y - xi_old.y) / dt });
        }
        left.x = xnew_left; // Update position
        combine_positions(x_combined, left.x, right.x, left.N, right.N);

        // Right chain
        Vec xnew_right = right.x;
        std::pair<double, int> result_right = gauss_seidel_minimize_with_barrier_global( xnew_right, right.xhat,
                                                                                         ...
                                                                                         x_combined, left.N,
                                                                                         max_sweeps, tol_abs, eta);
        double residual_right = result_right.first;
        int sweeps_right = result_right.second;
        ...
        right.x = xnew_right; // Update position
        combine_positions(x_combined, left.x, right.x, left.N, right.N);
        ...
        // Detect collision only if sign flips across frames
        static bool has_prev = false;
        bool collision = false;

        if (has_prev) {
            if (prev_signed_d * min_signed_d < 0.0) {
                collision = true;
            }
        } else {
            has_prev = true; // first frame observed
        }

        prev_signed_d = min_signed_d;
        ...
        std::cout << "Frame " << std::setw(4) << frame
                  ...
                  << " | sweepsL=" << std::setw(3) << sweeps_left
                  << " | sweepsR=" << std::setw(3) << sweeps_right
                  << " | signed_d=" << std::fixed << std::setprecision(6) << min_signed_d
                  << (collision ? "  COLLISION" : "")
                  << std::endl;
    }
