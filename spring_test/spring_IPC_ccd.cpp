...
// ==============================
Chain make_chain(Vec2 start, Vec2 end, int N, const std::vector<bool>& fixed_mask, double mass_value) {
    Chain c;
    c.N = N;
    c.x.resize(2 * N);
    c.v.assign(2 * N, 0.0);
...
std::vector<BarrierPair> build_barrier_pairs(const Vec& x_combined, const Vec& v_combined, int N_left, int N_right, double dt) {
    ...
    for (int i = 0; i < total_nodes; ++i) {
        ...
        AABB2 node_box{{min_x, min_y}, {max_x, max_y}};
    ...
    broad_phase_ccd(objects, [&](const Object& A, const Object& B) {
        ...
        bool invalid = (node == seg0) || (node == seg1) || (seg0 < N_left - 1 && node < N_left) || (seg0 >= N_left && node >= N_left);
        if (!invalid)
...
// Fixed node mask
std::vector<bool> fixed_first_only(int N) {
...
int main(){
    ...
    // Create chains
    Chain left  = make_chain({-1.0, 0.0}, {4.0, -5.0}, number_of_segments,
                             fixed_first_only(number_of_segments), 0.05); // y = -x - 1
    Chain right = make_chain({-1.5, 0.5}, {3.5, 0.5}, number_of_segments,
                             fixed_first_only(number_of_segments), 0.05); // y = 0.5
    ...
    for (int frame = 1; frame <= total_frame; ++frame) {
        ...
        auto [residual_left, iterations_left] = gauss_seidel_solver(xnew_left, left.xhat,
                                                                    ...
                                                                    dt, k_spring, g_accel,left.is_fixed,
        ...
        auto [residual_right, iterations_right] = gauss_seidel_solver( xnew_right, right.xhat,
                                                                       ...
                                                                       dt, k_spring, g_accel,right.is_fixed,
                                                                       barrier_pairs, dhat,x_combined,
