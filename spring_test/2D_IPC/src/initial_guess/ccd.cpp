...
namespace initial_guess::ccd {
    ...
    double global_safe_step(const Vec& x, const Vec& v,
        ...
        for (const auto& c : pairs) {
            ...
            omega = std::min(omega, step_filter::ccd::safe_step(
                xi, {dt*vi.x, dt*vi.y},
                xj, {dt*vj.x, dt*vj.y},
...
void CCDGuess::apply(Chain& left, Chain& right,
    ...
    BVHBroadPhase bp;
    auto pairs = bp.build_ccd_candidates(x_combined, v_combined, left.N, right.N, dt);
    double omega = initial_guess::ccd::global_safe_step(x_combined, v_combined, pairs, dt, eta);

    xnew_left  = left.x;
    xnew_right = right.x;
    for (int i = 0; i < left.N; ++i) {
        Vec2 xi = get_xi(left.x, i), vi = get_xi(left.v, i);
        set_xi(xnew_left,  i, {xi.x + omega*dt*vi.x, xi.y + omega*dt*vi.y});
    }
    for (int i = 0; i < right.N; ++i) {
        Vec2 xi = get_xi(right.x, i), vi = get_xi(right.v, i);
        set_xi(xnew_right, i, {xi.x + omega*dt*vi.x, xi.y + omega*dt*vi.y});
    }
