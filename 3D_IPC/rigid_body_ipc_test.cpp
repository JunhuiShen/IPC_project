        ...
        for (const int proxy_nodes : {3, 131}) {
            for (const int bodies : {7, 8, 9}) {
                ...
                for (int run = 0; run < 2; ++run) {
                    ...
                    params.max_global_iters = fixed ? 7 : 4; params.fixed_iters = fixed;
                    ...
                    params.node_box_update_count = fixed ? 3 : 1; params.use_parallel = true;
                    ...
                    omp_set_num_threads(run == 0 ? 1 : std::min(64, omp_get_num_procs()));
                    results[run] = global_gauss_seidel_solver_basic_rb(meshes[run], states[run], params,
                        coms[run], orientations[run], omega[run]);
                }
                if (fixed) {
                    EXPECT_EQ(results[0].iterations, 7);
                    EXPECT_EQ(results[1].iterations, 7);
                    EXPECT_FALSE(results[1].has_residual);
                }
