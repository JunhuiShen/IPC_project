...
void mixed_deformable_vertex_safe_step(const BroadPhase& broad_phase, std::vector<Vec3>& x, int vi, const std::function<Vec3(int)>& x_new_fn, double safety = 0.9, bool clip_ccd = true, bool use_ticcd = true, bool use_ogc = false, std::atomic<int>* clip_count = nullptr);
