...
namespace {
    ...
    void export_frame(const std::string& outdir, int frame, const std::vector<Vec3>& x, const std::vector<int>& tris, ExportFormat fmt, const std::vector<std::vector<int>>* color_groups) {
...
void export_nt_pairs_geo(const std::string& filename, const std::vector<Vec3>& x, const std::vector<NodeTrianglePair>& nt_pairs) {
    std::ofstream out(filename);
    if (!out) return;
...
void export_ss_pairs_geo(const std::string& filename, const std::vector<Vec3>& x, const std::vector<SegmentSegmentPair>& ss_pairs) {
    std::ofstream out(filename);
    if (!out) return;
