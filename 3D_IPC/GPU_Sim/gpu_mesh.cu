// gpu_mesh.cu
// Real CUDA implementation of the GPU mirror structs.
// Compile this instead of gpu_mesh_stub.cpp on a machine with a CUDA toolchain.
...
// --------------------------------------------------------------------------
// DeviceBuffer  (real CUDA: cudaMalloc / cudaMemcpy)
// --------------------------------------------------------------------------
...
void DeviceBuffer<T>::upload(const T* host_data, int n) {
    alloc(n);
    cudaMemcpy(ptr, host_data, static_cast<size_t>(n) * sizeof(T), cudaMemcpyHostToDevice);
}

template<typename T>
void DeviceBuffer<T>::download(T* host_data) const {
    cudaMemcpy(host_data, ptr, static_cast<size_t>(count) * sizeof(T), cudaMemcpyDeviceToHost);
}
