...
static PairSets pair_sets_from_broad(const BroadPhase3D& broad) {
...
static PairSets exact_close_pairs(const std::vector<Vec3>& x, const RefMesh& mesh, double d_hat) {
    ...
    for (int e = 0; e < ne; ++e) {
        for (int f = e + 1; f < ne; ++f) {
            if (share_vertex(edges[e], edges[f])) continue;
            const double d = segment_segment_distance(
                x[edges[e].a], x[edges[e].b], x[edges[f].a], x[edges[f].b], eps).distance;
...
TEST(BVH3Test, QueryReturnsExpectedHits) {
    ...
    boxes.emplace_back(Vec3(0.0, 0.0, 0.0), Vec3(1.0, 1.0, 1.0)); // 0
    boxes.emplace_back(Vec3(2.0, 2.0, 2.0), Vec3(3.0, 3.0, 3.0)); // 1
    boxes.emplace_back(Vec3(0.5, 0.5, 0.5), Vec3(1.5, 1.5, 1.5)); // 2
...
TEST(BroadPhase3DTest, SingleTriangleProducesNoSelfNodeTrianglePairs) {
...
TEST(BroadPhase3DTest, DetectsNodeTrianglePairFromOverlappingBoxes) {
...
TEST(BroadPhase3DTest, FiltersOutNodeTrianglePairsThatShareVertices) {
    const std::vector<Vec3> x = {Vec3(0.0, 0.0, 0.0), Vec3(1.0, 0.0, 0.0), Vec3(0.0, 1.0, 0.0)};
    const std::vector<Vec3> v(x.size(), Vec3::Zero());
    const RefMesh mesh = make_mesh(x, {{0, 1, 2}});
...
TEST(BroadPhase3DTest, DetectsSegmentSegmentPairFromOverlappingBoxes) {
    const std::vector<Vec3> x = {
...
TEST(BroadPhase3DTest, FiltersOutSegmentPairsThatShareVertices) {
    const std::vector<Vec3> x = {Vec3(0.0, 0.0, 0.0), Vec3(1.0, 0.0, 0.0), Vec3(0.0, 1.0, 0.0)};
    const std::vector<Vec3> v(x.size(), Vec3::Zero());
    const RefMesh mesh = make_mesh(x, {{0, 1, 2}});
...
TEST(BroadPhase3DTest, CCDCandidatesDetectFutureNodeTriangleOverlap) {
    const std::vector<Vec3> x = {
        Vec3(0.0, 0.0, 0.0), Vec3(1.0, 0.0, 0.0), Vec3(0.0, 1.0, 0.0),
...
TEST(BroadPhase3DTest, SingleMeshSelfCollisionFoldDetectsNonIncidentPairs) {
    const std::vector<Vec3> x = {
...
TEST(BroadPhase3DTest, CCDCandidatesDetectFutureSegmentSegmentOverlap) {
    const std::vector<Vec3> x = {
        Vec3(0.0, 0.0, 0.0), Vec3(1.0, 0.0, 0.0), Vec3(0.0, 1.0, 0.0),
...
TEST(BroadPhase3DTest, CCDCandidatesDetectTangentialSkimmingMotion) {
    const std::vector<Vec3> x = {
        Vec3(0.0, 0.0, 0.00), Vec3(1.0, 0.0, 0.00), Vec3(0.0, 1.0, 0.00),
...
TEST(BroadPhase3DTest, InitializeMatchesBruteForceReference) {
    std::vector<Vec3> x, v;
    RefMesh mesh;
    ...
    const PairSets ref = brute_force_candidates(x, v, mesh, dt, dhat, dhat, dhat);
    EXPECT_EQ(got.nt, ref.nt);
    EXPECT_EQ(got.ss, ref.ss);
}

TEST(BroadPhase3DTest, ThreeObjectsPopulateAllPairwiseInteractions) {
    std::vector<Vec3> x, v;
    RefMesh mesh;
    build_three_sheet_scene(x, v, mesh);

    BroadPhase3D broad;
    broad.initialize(x, v, mesh, 1.0, 0.04);

    const auto& nt = broad.nt_pairs();
    EXPECT_TRUE(contains_nt_pair(nt, 3, 0, 1, 2)); // object B vs A
    EXPECT_TRUE(contains_nt_pair(nt, 6, 3, 4, 5)); // object C vs B
    EXPECT_TRUE(contains_nt_pair(nt, 6, 0, 1, 2)); // object C vs A
}

TEST(BroadPhase3DTest, CCDCandidatesMatchBruteForceReferenceAtZeroPad) {
    std::vector<Vec3> x, v;
    RefMesh mesh;
    build_two_sheet_scene(x, v, mesh);
...
TEST(BroadPhase3DTest, BroadPhaseIsConservativeForPairsCloserThanDhat) {
    std::vector<Vec3> x, v;
    RefMesh mesh;
    build_two_sheet_scene(x, v, mesh);
...
TEST(BroadPhase3DTest, LargerDhatProducesSupersetOfPairs) {
    std::vector<Vec3> x, v;
    RefMesh mesh;
    build_two_sheet_scene(x, v, mesh);
    ...
    BroadPhase3D small_bp, large_bp;
...
TEST(BroadPhase3DTest, OutputPairsAreUnique) {
    std::vector<Vec3> x, v;
    RefMesh mesh;
    build_two_sheet_scene(x, v, mesh);
...
TEST(BroadPhase3DTest, EmptyMeshProducesNoPairs) {
    const std::vector<Vec3> x;
...
TEST(BroadPhase3DTest, TriangleFreeMeshProducesNoPairs) {
    const std::vector<Vec3> x = {Vec3(0.0, 0.0, 0.0), Vec3(1.0, 0.0, 0.0), Vec3(0.0, 1.0, 0.0)};
    const std::vector<Vec3> v(x.size(), Vec3::Zero());
    const RefMesh mesh = make_mesh(x, {});
...
TEST(BroadPhase3DTest, RefreshMatchesInitializeForBaselineRebuildImplementation) {
    std::vector<Vec3> x = {
        Vec3(0.0, 0.0, 0.0), Vec3(1.0, 0.0, 0.0), Vec3(0.0, 1.0, 0.0),
    ...
    a.refresh(x, v, mesh, 3, 1.0, 0.15, 0.15, 0.15);
...
TEST(BroadPhase3DTest, RefreshRemovesPairsWhenGeometrySeparates) {
    std::vector<Vec3> x = {
        Vec3(0.0, 0.0, 0.00), Vec3(1.0, 0.0, 0.00), Vec3(0.0, 1.0, 0.00),
    ...
    broad.refresh(x, v, mesh, 3, 1.0, 0.03, 0.03, 0.03);
