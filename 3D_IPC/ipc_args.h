...
struct IPCArgs3D : ArgParser {
    ...
    // --- solver ---
    int    max_iters     = 500;   // -
    double tol_abs       = 1e-6;  // N   (residual force inf-norm)
    double step_weight   = 1.0;   // -
    double d_hat         = 0.02;  // m   (barrier activation distance; 0 = off)
    ...
    IPCArgs3D() {
        add_double("fps",         fps,         30.0,       "[Hz] Output frames per second");
        ...
        add_double("mu",          mu,          10.0,       "[Pa] First Lame parameter (shear modulus)");
        add_double("lambda",      lambda,      10.0,       "[Pa] Second Lame parameter");
        add_double("density",     density,     1.0,        "[kg/m^3] Mass density");
        add_double("thickness",   thickness,   0.1,        "[m] Shell thickness");
        add_double("kB",          kB,          1e-2,       "[J] Bending stiffness (0 disables bending)");
        add_double("kpin",        kpin,        1e7,        "[N/m] Pin spring stiffness");
        add_double("gx",          gx,          0.0,        "[m/s^2] Gravity x-component");
        add_double("gy",          gy,          -9.81,      "[m/s^2] Gravity y-component");
        add_double("gz",          gz,          0.0,        "[m/s^2] Gravity z-component");

        add_int   ("max_iters",   max_iters,   500,        "[-] Max Gauss-Seidel iterations per frame");
        add_double("tol_abs",     tol_abs,     1e-6,       "[N] Absolute convergence tolerance (residual force)");
        add_double("step_weight", step_weight, 1.0,        "[-] Newton step damping factor");
        add_double("d_hat",       d_hat,       0.02,       "[m] Barrier activation distance (0 = off)");
        ...
        add_double("width",       width,       1.0,        "[m] Mesh width");
        add_double("height",      height,      1.0,        "[m] Mesh height");

        add_double("left_x",      left_x,      -0.75,      "[m] Left sheet origin x");
        add_double("right_x",     right_x,     0.75,       "[m] Right sheet origin x");
        add_double("sheet_y",     sheet_y,     0.20,       "[m] Shared sheet origin y");
        add_double("left_z",      left_z,      0.00,       "[m] Left sheet origin z");
        add_double("right_z",     right_z,     0.02,       "[m] Right sheet origin z");
