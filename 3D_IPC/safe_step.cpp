...
void mixed_deformable_vertex_safe_step(const BroadPhase& broad_phase, std::vector<Vec3>& x, int vi, const Vec3& raw_proposed_position, double safety, bool clip_ccd, bool use_ticcd, bool use_ogc) {
    ...
    if (vi < 0 || vi >= nv)
        throw std::out_of_range("mixed_deformable_vertex_safe_step: vertex is out of range");

    const AABB& box = bp_cache.node_boxes[vi];
    assert((x[vi].array() >= box.min.array()).all() && (x[vi].array() <= box.max.array()).all() && "mixed_deformable_vertex_safe_step: current position is outside its cached node box");
